...
- [ ] Create `UserManagementWidget` UI component with a Data Grid (`QTableWidget`).
- [ ] Implement "Add User" dialog (Student ID, Full Name).
- [ ] Implement "Edit User" dialog.
- [ ] Implement "Delete User" logic with confirmation warnings (and cascade delete or soft delete for related embeddings/attendance).
- [ ] Wire the UI to `UserRepository` to reflect real-time database changes.
