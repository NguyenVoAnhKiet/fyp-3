...
- Import `onnxruntime` before `PyQt5` on Windows. Keep that order in `main.py` and `tests/conftest.py`.
- `src/attendance_system/core/bootstrap.py` does **not** call `load_dotenv()`. Use `--database-path` or env fallback only.
...
- `FaceReferenceRepository._cache_all` is keyed by DB path; every write path must invalidate the cache.
- `attendance_records.user_id` is nullable and uses `ON DELETE SET NULL`.
- `_crop_face` uses different scales: attendance/enrollment liveness `2.7`, head-pose default `1.5`.
...
- `cryptography` is soft-dep; tests may skip encryption paths with `pytest.importorskip("cryptography.fernet")`.
- `pandas` and `openpyxl` are soft-deps for Excel export.
