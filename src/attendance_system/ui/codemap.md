...
| File | Role |
|---|---|
| `camera_worker_base.py` | **Base classes** — `CameraThreadBase(QThread)` (camera init, `_retry_read()` exponential backoff, `pause()`/`resume()`, `_detect_faces()` via YuNet, `_draw_bboxes()`, `_annotate_frame()` with QPainter, `_emit_display_frame()`) and `AIWorkerBase(QThread)` (queue+sentinel with maxsize=1 backpressure, `submit_task()` with auto `.copy()` on numpy arrays, `is_busy()`, circuit-breaker with `_MAX_CONSECUTIVE_FAILURES = 30`, `stop()` with drain+wait). Both define abstract `_process_frame()` for subclass override. **Expanded docstrings** — both classes now have detailed Google-style docstrings covering lifecycle, subclass hooks, signals, and error handling. `_running` flag has atomicity comment ("Atomic under CPython GIL"). `from pathlib import Path` moved from inline to top-level imports. |
| `camera_thread.py` | **Attendance camera** — `CameraThread(CameraThreadBase)` captures webcam frames, runs YuNet face detection, delegates liveness + recognition to `AIWorker(AIWorkerBase)` (inner QThread, uses `_COOLDOWN_SECONDS = 1.5` per-user cooldown, emits `recognition_result(result_type, user_id, full_name, liveness_score, similarity_score, matched_pose_label)` via arity-6 pyqtSignal). **Constructor** accepts hybrid liveness params (`hybrid_liveness_enabled`, `hybrid_voting_window`, `hybrid_boost_amount`, `recognition_interval`) with defaults from `defaults.py`, forwarded to `AIPipeline()`. **Consensus buffer** — `_consensus_buffer` (3-frame `deque`) with `_compute_consensus()` requiring 2/3 majority before final emission, suppressing transient flicker. **AIWorker.stop()** calls `self._pipeline.reset_tracker()` before `super().stop()`. **AIWorker._process_frame()** passes its own `self._ai_frame_counter` to `run_attendance()` (not the camera-loop `frame_counter`). No frame mirroring. |
...
| `user_mode_view.py` | IDLE/ACTIVE dual-state widget. IDLE: subject + class input form with admin login shortcut. ACTIVE: camera feed, stats grid (success/spoof/unrecognized/time), attendance sidebar list, end-session button. Creates and controls `CameraThread` (passes hybrid liveness params from `SystemConfig`: `hybrid_liveness_enabled`, `hybrid_voting_window`, `hybrid_boost_amount`, `recognition_interval`), handles its `recognition_result` signal (arity 6). **Freeze overlay** — after first recognition, pauses camera and shows success overlay for configurable seconds with optional sound cue. Connects to `timezone_signals.timezone_changed` to re-render the sidebar mid-session. Clears stale camera pixmap and resets placeholder text on session start AND end (single helper `_reset_camera_preview()`). **Stat card refactor** — `_make_stat_card()` now returns `(QFrame, QLabel)` tuple; labels stored explicitly (`self._stat_success_label`, etc.). Imports `DEFAULT_ATTENDANCE_FREEZE_SECONDS` from `core.defaults` (replaces magic number `4`). Three bare `pass` blocks replaced with `logger.warning()` calls (sidebar re-population, time parsing, timezone sidebar repopulation). |
...
11. **Hybrid liveness params in CameraThread constructor** — `CameraThread.__init__()` accepts `hybrid_liveness_enabled`, `hybrid_voting_window`, `hybrid_boost_amount`, and `recognition_interval`, all defaulting from `defaults.py`. These are forwarded to `AIPipeline()` at construction. `UserModeView._start_session()` reads them from the resolved `SystemConfig` and passes them through.

12. **AIWorker tracker reset on stop** — `AIWorker.stop()` calls `self._pipeline.reset_tracker()` before `super().stop()`. This clears the liveness tracker's EMA/IoU state so a subsequent session does not inherit stale buffer state from the previous session.

13. **Self-owned `_ai_frame_counter`** — `AIWorker._process_frame()` maintains its own `_ai_frame_counter` (incremented per-processed frame), which it passes to `run_attendance()` instead of the camera-loop `frame_counter`. This keeps frame numbering consistent in the AI pipeline even when frames are dropped due to backpressure or when the camera loop runs at a different rate.

14. **Recognition consensus buffer** — `CameraThread._consensus_buffer` is a 3-frame `deque` of `(mapped_user_id, result_type)` tuples. `_compute_consensus()` requires a 2/3 majority before the final `recognition_result` is emitted. `"success"` votes contribute to the user-ID majority; `"spoof"` and `"unrecognized"` are counted separately (no single-spoof frame triggers a false alert).

15. **Enrollment dropdown data is a tuple** — `EnrollmentWidget._user_dropdown.currentData()` returns `(user_id: int, is_reenroll: bool, full_name: str)` instead of a plain `user_id`. The re-enroll flag drives confirmation dialog and success message logic in `_start_enrollment()` / `_finalize_enrollment()`. Access the first element (`item_data[0]`) for the user ID.
